...
# Whether Skript should compress backup files (configs and variables).
# If true, those files will be zipped to save disk space.
...
# Disables the "... its name should contain '::'. Having a single ':' does nothing!" warnings for variable names.
...
# This list defines the tags that Skript will process by default, without the usage of the "formatted" expression.
# By default, this simply includes color and decoration applying tags.
# Tags are listed by their names or internal groups. They should be separated by only a comma and a space (', ').
...
# Enables type properties, which allows addons to hook into common syntaxes like `if x is empty` or `name of x`.
# You should only disable this if you are encountering issues with Skript being unable to figure out the properties,
# and you should make an issue on SkriptLang's GitHub.
